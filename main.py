...
def view_student_list():
    if session.get('teacher_logged_in')==True:
        ...
        return render_template("user/student_list.html", student_lists=student_details,logged_teacher=session.get('logged_teacher'),schoolyear=schoolyear)
    else:
        return false_login()
...
@app.route('/view_students_by_schedule/<int:schedule_id>', methods=["POST", "GET"])
def view_students_by_schedule(schedule_id):
    if session.get('teacher_logged_in')==True:
        ...
        return render_template("user/students_by_schedule.html",student_lists=student_lists, sched_info=fetched_sched, logged_teacher= logged_teacher)
    else:
        return false_login()
        ...
        while True:
            ...
            for (top, right, bottom, left), face_encoding in zip(faces, encodings):
                ...
                if True in matches: 
                    ...
                    existinfo = cursor.fetchone()
                    if existinfo:
                        student_name = existinfo[1]
                        student_id_for_record = student_id
                        name = student_name
                        color = (0, 255, 0)
                        ...
                        if name not in logged_students:
                            ...
                            guardian_contact = contact[0] if contact else None
                            ...
                            cursor.execute("""
                                ...
                                WHERE schedule_id = %s
                            """, (schedule_id,))
                            sched_time = cursor.fetchone()
                            schedule, start_time, end_time = sched_time if sched_time else (None, None)
                            print (schedule)

                            time_in = datetime.now().strftime("%H:%M:%S") #gets the time now
                            now = datetime.strptime(time_in, "%H:%M:%S").time() #strips the time 
                            day_now = datetime.now().strftime("%A") #get the full name of dat today
                            # Check if student already logged in CSV
                            already_logged = False
                            if os.path.exists(csv_path):
                                with open(csv_path, "r") as f:
                                    for row in csv.reader(f):
                                        if row and row[0] == str(student_id_for_record):
                                            already_logged = True
                                            break

                            if already_logged:
                                status_text = f"{student_name} already logged"
                            elif start_time and end_time:
                                start_str = str(start_time)
                                end_str = str(end_time)

                                if len(start_str) == 5:  # example "18:05"
                                    start_str += ":00"
                                if len(end_str) == 5:
                                    end_str += ":00"
                                ...
                                if day_now==schedule:
                                    if now < start:
                                        ...
                                        status_text = f"{student_name} not logged (past class time)"
                                        status = None
                                        color = (0, 0, 255)
                                    ...
                                    if status:
                                        ...
                                        with open(csv_path, "a", newline="") as f:
                                            csv.writer(f).writerow([student_id_for_record, student_name, time_in, status])
                                        logged_students.add(name)
                                        status_text = f"{student_name} marked {status}"

                                        # Send SMS notification
                                        if guardian_contact:
                                            send_attendance_sms(guardian_contact, student_name, time_in)
                                else:
                                    status_text = "Schedule is not Today"
                            else:
                                status_text = "Class time not found"
                    
                    else:
                        color = (0, 0, 255)
                        status_text = f"Not in this Schedule, scan next student"
                        name = "Unknown"
